...
def get_unsched_pods(node_pool_name, secret):
   ...
   response = Popen(['kubectl config set-credentials oke-autoscaler-cf --token=' + secret],
   ...
   #   - set context..
   response = Popen(['kubectl config set-context --current --user=oke-autoscaler-cf'],
                    shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
                   )
   output, errors = response.communicate()
   logging.info("Popen Output, kubectl config: " + output)
